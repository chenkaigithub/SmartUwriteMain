...
namespace BIMTClassLibrary.dynamicClass
{
    public class ClassHelper
    {
        ...
        private static void AddPropertyToTypeBuilder(TypeBuilder myTypeBuilder, List<CustPropertyInfo> lcpi)
        {
            ...
            {
                //定义字段。
                string _strName = cpi.FieldName;
                Type _type = Type.GetType(cpi.Type);
                FieldAttributes _fa = FieldAttributes.Private;
                FieldBuilder customerNameBldr = myTypeBuilder.DefineField(_strName,
                                                                          _type,
                                                                          _fa);
                customerNameBldr.SetConstant(DefaultForType(Type.GetType(cpi.Type)));
                //定义属性。
                ...
                custNamePropBldr.SetConstant(DefaultForType(Type.GetType(cpi.Type)));
                //定义Get方法。
